...
class KuaishouPublisher(BasePublisher):
    ...
    def __init__(self, config: dict):
        super().__init__(config, "kuaishou")
    ...
    def publish(self, title: str, content: str, image_path: str = None, tags: list = None, is_html: bool = False) -> bool:
        try:
            ...
            if "login" in self.page.url or "passport" in self.page.url:
                ...
                while "login" in self.page.url or "passport" in self.page.url:
                    time.sleep(2)
            ...
            # 输入标题
            title_input = self.page.locator('[placeholder*="标题"], input[placeholder*="标题"]').first
            title_input.click()
            title_input.fill(title)
            time.sleep(0.5)

            # 输入正文
            content_input = self.page.locator('[placeholder*="正文"], [placeholder*="内容"], .editor-content').first
            content_input.click()
            if is_html:
                # 如果是 HTML，直接注入
